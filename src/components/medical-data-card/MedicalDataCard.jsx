...
function normalizeAnswers(questions, answers) {
  // Garante que answers sempre tenha o mesmo tamanho e formato esperado
  return questions.map((_, idx) => {
    const ans = answers?.[idx];
    if (!ans) return { value: null, extra: "" };
    if (typeof ans.value === "boolean") {
      return {
        value: ans.value === true ? "sim" : "nao",
        extra: ans.value === true ? ans.extra || "" : "",
      };
    }
    // Se já está no formato string
    return {
      value: ans.value,
...
const MedicalDataCard = ({ questions, onChange, answers: initialAnswers }) => {
  const [answers, setAnswers] = useState(
    normalizeAnswers(questions, initialAnswers)
  );

  // Atualiza o estado interno só se mudou de verdade
  useEffect(() => {
    const normalized = normalizeAnswers(questions, initialAnswers);
    // Só atualiza se for diferente
    if (JSON.stringify(normalized) !== JSON.stringify(answers)) {
