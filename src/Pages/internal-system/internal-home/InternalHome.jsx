    ...
    return (
        <div className="grid md:grid-cols-2 gap-6 max-w-7xl mx-auto p-4">
            ...
            <div className="bg-gray-100 rounded-xl p-6 flex flex-col items-center gap-3 shadow-md">
                {/* Título e Select */}
                {/* ALTERAÇÃO: Layout em coluna no mobile (flex-col) e em linha a partir de telas pequenas (sm:flex-row) */}
                <div className="flex flex-col sm:flex-row items-center justify-center w-full mb-2 gap-3">
                    <h2 className="text-xl font-bold text-gray-800 mb-0">
                        Meta
                    </h2>
                    <select
                        className="p-2 border border-gray-300 rounded bg-[#EDEDED] w-full max-w-xs"
                        value={selectedTagId}
                ...
                {/* Gráfico */}
                {/* ALTERAÇÃO: Altura ajustada para mobile (h-56) e telas maiores (sm:h-64) */}
                <div className="relative w-full h-56 sm:h-64">
                    <Doughnut data={chartData} options={chartOptions} />
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <span className="text-xs text-gray-500">
                            R${" "}
                            {/* ALTERAÇÃO: Tamanho da fonte ajustado para mobile (text-xl) e telas maiores (sm:text-2xl) */}
                ...
                {/* Legenda */}
                {/* ALTERAÇÃO: Layout em coluna no mobile (flex-col) e em linha a partir de telas pequenas (sm:flex-row) */}
                <div className="flex flex-col sm:flex-row items-center mt-4 text-sm gap-2 sm:gap-6">
            ...
            <div className="bg-gray-100 rounded-xl p-6 shadow-md">
                 {/* ALTERAÇÃO: Layout em coluna no mobile (flex-col) e em linha a partir de telas pequenas (sm:flex-row) */}
                <div className="flex flex-col items-start sm:flex-row sm:items-center sm:justify-between mb-4 gap-4 sm:gap-0">
                    ...
                    {userRole === "EXECUTIVO" ||
                        (userRole === "DIRETOR" && (
                            <button
                                className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition duration-150 text-sm"
                        ...
                        return (
                            ...
                            >
                                <div className="py-2 px-4 flex items-center gap-3">
                                    ...
                                    />
                                    {/* ALTERAÇÃO: Removida a largura máxima fixa para permitir que o texto se ajuste melhor */}
